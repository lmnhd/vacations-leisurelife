...
export function CampaignLandingPageClaude({ landing }: CampaignLandingPageClaudeProps) {
    const p = buildPalette(landing);

    const galleryImages = landing.galleryImages.filter((img) => img.url.trim().length > 0);
    const getGalleryImage = (index: number) =>
        galleryImages.length > 0 ? galleryImages[index % galleryImages.length] : null;

    const primaryHref =
        landing.links.booking && landing.ctas.primary.mode === 'BOOK_NOW' && !landing.ctas.primary.disabled
            ? landing.links.booking
            : '#save-your-place';

    const secondaryHref =
        landing.links.booking && landing.ctas.secondary.mode === 'BOOK_NOW' && !landing.ctas.secondary.disabled
            ? landing.links.booking
            : '#save-your-place';

    const heroGradient = `linear-gradient(to top, ${landing.surfaceColor} 0%, ${landing.surfaceColor}99 45%, ${landing.surfaceColor}22 100%)`;

    const voyageLabels = new Set(['Sailing', 'Ship', 'Departure Port', 'Destination', 'Duration']);
    ...
    const voyageColClass = voyageFacts.length <= 3
        ? 'grid-cols-1 sm:grid-cols-3'
        : voyageFacts.length === 4 ? 'grid-cols-2 lg:grid-cols-4' : 'grid-cols-2 lg:grid-cols-5';
    ...
    return (
        ...
        >
            ...
            <section className="relative flex min-h-screen flex-col justify-end overflow-hidden">
                {landing.heroImage?.url ? (
                    ...
                    <div
                        ...
                        style={{ background: `linear-gradient(160deg, ${landing.surfaceColor}, #000)` }}
                    />
                )}
                ...
                <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pb-14 pt-28 md:px-6 lg:px-8">
                    <div className="grid items-end gap-10 lg:grid-cols-[1fr_340px]">
                        <div className="grid gap-6">
                            <div className="h-0.5 w-20" style={{ backgroundColor: landing.accentColor }} />
                            <div>
                                <p
                            ...
                            <div className="flex flex-wrap gap-3 pt-1">
                                {landing.ctas.primary.disabled ? (
                                    ...
                                    <Button
                                        ...
                                        style={{ backgroundColor: landing.accentColor, color: p.onAccentText }}
                                    >
                                        <a
                                            href={primaryHref}
                                ...
                                {landing.ctas.secondary.disabled ? (
                                    <Button
                                        ...
                                        disabled
                                        className="h-12 rounded-none px-7 text-base opacity-40"
                                        style={{ borderColor: landing.accentColor, color: p.bodyText }}
                                    >
                        ...
                        <div className="hidden lg:block">
                            <p className="text-sm leading-7" style={{ color: p.bodyText, opacity: 0.6 }}>
            ...
            */}
                        ...
                        <div className="mx-auto max-w-7xl px-4 md:px-6 lg:px-8">
                            <div className={`grid ${voyageColClass}`}>
                                {voyageFacts.map((fact, idx) => (
                                    <div key={fact.label} className="px-6 py-7"
                                        style={idx < voyageFacts.length - 1 ? { borderRight: `1px solid ${p.border}` } : undefined}>
                                        <p className="text-xs uppercase tracking-widest" style={{ color: landing.accentColor }}>{fact.label}</p>
                                        <p className="mt-2 text-xl font-semibold" style={{ color: p.bodyText }}>{fact.value}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-2">
                                {groupFacts.map((fact, idx) => (
                                    <div key={fact.label} className="px-6 py-5"
                                        style={idx < groupFacts.length - 1 ? { borderRight: `1px solid ${p.border}` } : undefined}>
                                        <p className="text-xs uppercase tracking-widest opacity-50" style={{ color: p.bodyText }}>{fact.label}</p>
                                        <p className="mt-1 text-base font-semibold opacity-80" style={{ color: p.bodyText }}>{fact.value}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="grid gap-10 lg:grid-cols-2">
                    ...
                    <div className="grid gap-8">
                        <div className="grid gap-4">
                            ...
                            <p className="leading-8" style={{ color: p.bodyText, opacity: 0.7 }}>
                        ...
                        <div className="grid gap-3">
                            {landing.story.whyJoinNow.map((reason) => (
                                <div key={reason} className="flex gap-3">
                                    ...
                                    />
                                    <p className="text-sm leading-7" style={{ color: p.bodyText, opacity: 0.8 }}>
                                        {reason}
                                    </p>
                                </div>
                            ))}
                            ...
                            <>
                                ...
                                <div className="grid gap-3">
                                    {landing.experienceBullets.map((bullet) => (
                                        ...
                                        >
                                            <p className="text-sm leading-7" style={{ color: p.bodyText, opacity: 0.7 }}>
                                                {bullet}
                                            </p>
                                        </div>
                                    ))}
                        ...
                        <p className="text-sm leading-7 lg:hidden" style={{ color: p.bodyText, opacity: 0.5 }}>
                            {landing.elevatorPitch}
                        </p>
                    </div>
                    ...
                    <div className="grid gap-6">
                        ...
                        >
                            ...
                            <p className="mt-2 text-sm leading-7" style={{ color: p.bodyText, opacity: 0.6 }}>
                                {landing.threshold.detail}
                            </p>
                            <div className="mt-5 grid gap-2">
                                <div className="flex justify-between text-xs" style={{ color: p.bodyText, opacity: 0.5 }}>
                                    <span>{landing.threshold.joinedEntries} cabin requests</span>
                                    <span>{landing.threshold.requiredCabins} needed to launch</span>
                                </div>
                                <div className="h-1.5" style={{ backgroundColor: p.progressTrack }}>
                                    <div
                                        className="h-full"
                                        style={{
                            ...
                            <div className="mt-5 grid grid-cols-2 gap-4">
                                <div className="p-4" style={{ border: `1px solid ${p.border}` }}>
                                    <p className="text-xs" style={{ color: p.bodyText, opacity: 0.5 }}>Guests represented</p>
                                    <p className="mt-1 text-2xl font-semibold" style={{ color: landing.accentColor }}>
                                        {landing.threshold.joinedPassengers}
                                    </p>
                                </div>
                                <div className="p-4" style={{ border: `1px solid ${p.border}` }}>
                                    <p className="text-xs" style={{ color: p.bodyText, opacity: 0.5 }}>Group progress</p>
                                    <p className="mt-1 text-2xl font-semibold" style={{ color: landing.accentColor }}>
                        ...
                        <div
                            ...
                            style={{ border: `1px solid ${p.border}`, backgroundColor: p.cardBg }}
                        >
                            <p className="text-xs uppercase tracking-widest" style={{ color: landing.accentColor }}>
                                {landing.pricing.sourceLabel}
                            </p>
                            <div className="mt-4 grid grid-cols-2 gap-6">
                                <div>
                                    <p className="text-xs" style={{ color: p.bodyText, opacity: 0.5 }}>Starting from</p>
                                    <p className="mt-1 text-3xl font-semibold" style={{ color: landing.accentColor }}>
                                        {landing.pricing.startingPriceLabel}
                                    </p>
                                </div>
                                <div className="pl-4" style={{ borderLeft: `1px solid ${p.border}` }}>
                                    <p className="text-sm leading-7" style={{ color: p.bodyText, opacity: 0.6 }}>
                            ...
                            {getGalleryImage(0) && (
                                <div className="relative mt-5 h-28 overflow-hidden">
                                    ...
                                    />
                                    <div className="absolute inset-0" style={{ backgroundColor: p.imageOverlay }} />
                                </div>
                            )}
                            <p className="mt-4 text-xs" style={{ color: p.bodyText, opacity: 0.4 }}>No payment is taken on this page.</p>
                        </div>

                        {/* Trust */}
                        <div className="grid gap-3 p-5" style={{ border: `1px solid ${p.border}` }}>
                            {landing.trustBullets.map((bullet) => (
                                <p key={bullet} className="text-xs leading-6" style={{ color: p.bodyText, opacity: 0.5 }}>
                                    {bullet}
                                </p>
                            ))}
                        </div>
                ...
                <Tabs defaultValue="expect" className="grid gap-8">
                    ...
                    >
                        <TabsTrigger
                            ...
                            style={{ borderRight: `1px solid ${p.border}`, color: p.bodyText }}
                        >
                            What To Expect
                        </TabsTrigger>
                        <TabsTrigger
                    ...
                    <TabsContent value="expect" className="grid gap-5 lg:grid-cols-2">
                        <div className="grid gap-4">
                            {landing.story.whatToExpect.map((item, idx) => (
                                <div key={item} className="flex gap-4 p-4" style={{ border: `1px solid ${p.border}` }}>
                                    <span className="mt-0.5 text-xl font-semibold tabular-nums leading-none" style={{ color: landing.accentColor, opacity: 0.45 }}>
                                        {String(idx + 1).padStart(2, '0')}
                                    </span>
                                    <p className="text-sm leading-7" style={{ color: p.bodyText, opacity: 0.8 }}>{item}</p>
                                </div>
                            ))}
                        </div>
                        <div className="grid gap-4">
                            {getGalleryImage(2) && (
                                <div className="relative h-44 overflow-hidden">
                                    <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${getGalleryImage(2)?.url})` }} />
                                </div>
                            )}
                            {landing.experienceBullets.map((bullet) => (
                                <div key={bullet} className="p-4" style={{ border: `1px solid ${p.border}`, backgroundColor: p.cardBg }}>
                                    <p className="text-sm leading-7" style={{ color: p.bodyText, opacity: 0.7 }}>{bullet}</p>
                                </div>
                            ))}
                        </div>
                    </TabsContent>

                    <TabsContent value="how-it-works" className="grid gap-5 lg:grid-cols-2">
                        <div className="grid gap-4">
                            {landing.story.howItWorks.map((step) => (
                                <div key={step.title} className="p-5" style={{ border: `1px solid ${p.border}` }}>
                                    <p className="font-semibold" style={{ color: p.bodyText }}>{step.title}</p>
                                    <p className="mt-2 text-sm leading-7" style={{ color: p.bodyText, opacity: 0.6 }}>{step.body}</p>
                                </div>
                            ))}
                        </div>
                        <div className="grid gap-4">
                            {getGalleryImage(3) && (
                                <div className="relative h-44 overflow-hidden">
                                    <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${getGalleryImage(3)?.url})` }} />
                                </div>
                            )}
                            {landing.trustBullets.map((bullet) => (
                                <div key={bullet} className="p-4" style={{ border: `1px solid ${p.border}`, backgroundColor: p.cardBg }}>
                    ...
                    <TabsContent value="faq" className="grid gap-4 lg:grid-cols-2">
                        {landing.faq.map((item) => (
                            <div key={item.question} className="p-5" style={{ border: `1px solid ${p.border}` }}>
                                <p className="font-semibold" style={{ color: p.bodyText }}>{item.question}</p>
                                <p className="mt-2 text-sm leading-7" style={{ color: p.bodyText, opacity: 0.6 }}>{item.answer}</p>
                            </div>
                        ))}
                    </TabsContent>
                </Tabs>
            ...
            <section
                id="save-your-place"
                style={{ borderTop: `1px solid ${p.border}`, backgroundColor: p.cardBg }}
            >
                <div className="mx-auto max-w-7xl px-4 py-16 md:px-6 lg:px-8">
