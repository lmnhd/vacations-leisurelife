...
const cbpickTestData = {
    ...
    elsepay: 'What else you will have to pay: rate includes port charges. Government tax and air additional.',
    ...
    other: 'Other details: Alaska early savings for 2024! Reserve today and receive prepaid gratuities, beverage package, WIFI and onboard credit on select dates. Sail along the inside passage and experience the great villages of the Gold Rush. Onboard Alaska naturalist along with Princess hospitality, fine dining and piazza shows. ALSO AVAILABLE Cruise Tour exclusive on Sapphire Princess, 4-nights land, 7-night cruise. Ask us all about it!  800.827.7779.'
  }
...
export default async function CBDestinationPicksTiles() {
  ...
  const sortedDestinations: sortedDestination[] = sortByDestination(__destinationPicks);
  ...
  async function formatPorts(ports: string) {
   const functionId = 'formatPorts'
    const instructions = "from the following text, which ports are included in this cruise? Make your answer 6 words or less:"
    
    const response = await aiAssistBackOff(instructions,ports,componentId,functionId);
    return response
  }
  async function formatDescription(info: string) {
   const functionId = 'formatDescription'
    const instructions = "summarize the following text in 50 characters or less. Do not make reference to the text and exclude phone numbers and call now statements, just summarize trip."
    
    const response = await aiAssistBackOff(instructions,info,componentId,functionId);
    return response || 'bad response'
  }
  async function formatPrice(info: string) {
    const functionId = 'formatPrice'
     const instructions = "tell me the lowest price mentioned in the following text in one word"
  ...
  async function mapTile(pick:CBPickData){
    ...
    const hasOptions:boolean = pick.other.includes('onboard credit') || pick.other.includes('dining') || pick.other.includes('beverage') || pick.other.includes('WIFI')
    const promotionTileProps:PromotionTileProps = {
        imageSrc:(await pexelmachine(3,pick.destination.replace('Destination:','').replace(' ','') + ' vacation' ))[0].srcMedium,
        alt:pick.destination,
        day:pick.when.replace('When:','').replace(' ',''),
        port: await formatPorts(pick.go.replace('Where you go:','').replace(' ','')),
        header1:pick.destination.replace('Destination:','').replace(' ',''),
        header2:pick.what.replace('What:','').replace(' ',''),
        description:await formatDescription(pick.other.replace('Other details:','').replace(' ','')),
        price:{perPerson: await formatPrice(pick.price.replace('Prices:','').replace(' ','')) + 
      '(*starting)'},
        toolTips: hasOptions ? {freeDining:pick.other.includes('dining'),
  ...
  async function mapTileData(sortedDests:any[]){
   return sortedDests.map(async (dest:sortedDestination,index) => {
    ...
    return <div key={index} className="flex flex-col ">
      <h3 className="my-2 text-lg font-bold text-center">{dest.destination}</h3>
