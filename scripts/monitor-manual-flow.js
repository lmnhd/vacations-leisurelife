"use strict";
/**
 * monitor-manual-flow.ts
 * Opens browser at search results. YOU navigate manually.
 * Captures every network request + URL change for comparison.
 */
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var playwright_1 = require("playwright");
var fs = require("fs");
var path = require("path");
var STATE_FILE = path.join(process.cwd(), '.playwright-state.json');
var CB_AGENT_TOOLS_URL = 'https://bookings.cbagenttools.com';
function monitor() {
    return __awaiter(this, void 0, void 0, function () {
        function attachMonitoring(p) {
            p.on('request', function (req) {
                var _a;
                // Only capture booking-related requests
                if (req.url().includes('cbagenttools') || req.url().includes('nitroapi')) {
                    var entry = {
                        time: new Date().toISOString(),
                        method: req.method(),
                        url: req.url(),
                        postData: (_a = req.postData()) !== null && _a !== void 0 ? _a : ''
                    };
                    capturedRequests.push(entry);
                    console.log("[REQ] ".concat(req.method(), " ").concat(req.url().substring(0, 100)));
                }
            });
            p.on('framenavigated', function (frame) {
                if (frame === p.mainFrame() && frame.url() !== 'about:blank') {
                    var entry = { time: new Date().toISOString(), url: frame.url() };
                    urlChanges.push(entry);
                    console.log("[NAV] ".concat(frame.url()));
                }
            });
        }
        var browser, context, capturedRequests, urlChanges, page;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, playwright_1.chromium.launch({
                        headless: false,
                        executablePath: 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe',
                        args: ['--no-sandbox', '--disable-blink-features=AutomationControlled']
                    })];
                case 1:
                    browser = _a.sent();
                    return [4 /*yield*/, browser.newContext({
                            storageState: fs.existsSync(STATE_FILE) ? STATE_FILE : undefined,
                            viewport: { width: 1920, height: 1080 }
                        })];
                case 2:
                    context = _a.sent();
                    capturedRequests = [];
                    urlChanges = [];
                    // Attach monitoring to any new tab that opens
                    context.on('page', function (newPage) {
                        console.log('[Monitor] New tab opened:', newPage.url());
                        attachMonitoring(newPage);
                    });
                    return [4 /*yield*/, context.newPage()];
                case 3:
                    page = _a.sent();
                    attachMonitoring(page);
                    return [4 /*yield*/, page.goto(CB_AGENT_TOOLS_URL, { waitUntil: 'domcontentloaded' })];
                case 4:
                    _a.sent();
                    console.log('\n===========================================');
                    console.log('GO! Click Book Now, fill guest info, click Continue.');
                    console.log('Monitoring all requests. Waiting 5 minutes...');
                    console.log('===========================================\n');
                    // Wait 5 minutes
                    return [4 /*yield*/, page.waitForTimeout(5 * 60 * 1000)];
                case 5:
                    // Wait 5 minutes
                    _a.sent();
                    // Save everything
                    fs.writeFileSync('manual-flow-requests.json', JSON.stringify(capturedRequests, null, 2));
                    fs.writeFileSync('manual-flow-urls.json', JSON.stringify(urlChanges, null, 2));
                    console.log("\nSaved ".concat(capturedRequests.length, " requests to manual-flow-requests.json"));
                    console.log("Saved ".concat(urlChanges.length, " URL changes to manual-flow-urls.json"));
                    return [4 /*yield*/, browser.close()];
                case 6:
                    _a.sent();
                    return [2 /*return*/];
            }
        });
    });
}
monitor().catch(console.error);
