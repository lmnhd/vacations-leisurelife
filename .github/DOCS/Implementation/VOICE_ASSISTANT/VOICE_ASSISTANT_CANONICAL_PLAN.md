...
**Documented deviation - Agents SDK.** The assignment prefers `RealtimeAgent`/`RealtimeSession` from `@openai/agents/realtime`. Every published `@openai/agents` version that supports the GA contract declares a `zod@^4` peer dependency and crashes at import against this repository's `zod@3.25.x` (verified empirically: `TypeError ... zod/v3/types.cjs` from `@openai/agents-core/dist/types/protocol.js`). Upgrading the whole workspace to zod 4 would destabilize the Booking Assistant/deals contracts mid-flight and is out of scope for this assignment. Decision: the Next app uses a first-party typed transport wrapper (`lib/voice/realtime-transport.ts`) implementing the exact GA contract above; the standalone telephony service (own `package.json`, deployed to Render) is free to adopt `@openai/agents` + zod 4 later. Revisit when the repo migrates to zod 4.
