# Klaviyo Live Test Plan â€” Full End-to-End

> **Status: Outline.** This document is the skeleton for the end-to-end live test pass we will run once Phases 1â€“5 of [KLAVIYO_EMAIL_FLOW_PLAN.md](./KLAVIYO_EMAIL_FLOW_PLAN.md) are all implemented. Each section is a placeholder for the actual test recipe that should be filled in close to test day, when the in-Klaviyo flow IDs, template versions, and audience filters are known.
>
> The intent of this plan is to certify the **app â†’ Klaviyo â†’ guest inbox** pipeline as a whole â€” wiring, payload, template rendering, deliverability, suppression, and analytics â€” rather than re-test the unit-level payload builders (already covered by `lib/campaigns/email/__tests__/`).
...
- [ ] Verify all metric names match `KLAVIYO_METRIC_NAMES` (frozen â€” diff against `lib/campaigns/email/email-event-types.ts`).
...
- [ ] Seed a clean test campaign (see Â§3).
...
- [ ] Verify the landing page renders (hero, threshold, CTAs) â€” this validates `hero_image_url` and `landing_page_url` will look right in email.
- [ ] Verify the test segment in Klaviyo now contains the seeded profiles.

---

## 4. Test Pass â€” Phase 1 (Three Nurture Emails)
...
- [ ] Verify the warnings list is empty for the matched campaign (any warning indicates incomplete test data â€” fix the data, not the email).
...
## 5. Test Pass â€” Phase 2 (Pre-Booking Lifecycle)
...
- [ ] Re-verify the safe-claim phrase (`threshold_met_claim`) is present in the rendered email â€” and that no template copy implies space is "secured."
...
### 5.2 `manifest_requested` (legacy, skip in current Cruise Brothers flow)

- [ ] This stage belonged to the older manifest handoff. Skip it for the current booking model.
- [ ] If a legacy deployment still uses it, keep the existing broadcast and CTA checks in place.

### 5.3 `manifest_reminder` (legacy, skip in current Cruise Brothers flow)

- [ ] This stage belonged to the older manifest handoff. Skip it for the current booking model.
- [ ] If a legacy deployment still uses it, keep the existing filter checks in place.
...
## 6. Test Pass â€” Phase 3 (Post-Booking Core + Scheduler)
...
- [ ] Re-save the same booking with different notes. Confirm `convertedNow: false`, `bookingConfirmedEmailFired: false` â€” no duplicate send.
- [ ] Ledger: one `converted` row + one `nurture_sent` row with `metadata.stage=booking_confirmed` (no second send on the re-save).

### 6.2 Scheduler â€” `travel_prep` and `final_countdown`
...
- [ ] From `/tests/klaviyo-emails` â†’ Scheduler Run panel: trigger a dry run for the test campaign with a `today` override. Confirm the response panel renders the plan + dispatched arrays.
...
- [ ] PATCH the campaign with a `finalItineraryUrl` (previously null). Confirm the broadcast fires to all `converted=true` leads only â€” non-converted leads in the test segment receive nothing.
...
- [ ] Confirm the Phase 3 flows have a Klaviyo-side filter on `campaign_status âˆˆ {CONVERTED, THRESHOLD_MET}` so an unconverted waitlist lead can never accidentally enter a post-booking flow.
...
## 7. Test Pass â€” Phase 4 (Change Notifications)

### 7.1 Severity matrix

For each severity in `critical â†’ high â†’ medium â†’ low â†’ positive`:
...
- [ ] Open one seeded inbox: the email arrived, subject reflects severity, the comparison block shows `previousValue` â†’ `newValue` correctly, the action module renders only when `actionRequired=true`.
- [ ] Confirm the Klaviyo flow branched on `event.severity` â€” inspect the live Klaviyo flow journey for the recipient.
- [ ] For `critical` specifically:
  - [ ] Confirm `smsDispatched > 0` for any seeded lead with a phone number on file.
  - [ ] Confirm `support_contact` renders in the email (other severities should NOT show this property â€” the orchestrator suppresses it server-side, but verify the rendered template too).
...
- [ ] File a `cancellation` change at `critical` severity. Confirm the email does NOT bury the cancellation language (plan Â§8 copy rule: "Never bury cancellation language").
- [ ] Confirm refund language is visible above the fold if applicable.

### 7.3 Klaviyo flow reminders

- [ ] Confirm the Klaviyo flow itself is configured with a 48h reminder branch on `severity=critical` and a 72h branch on `severity=high`. The app does NOT trigger these â€” they're delay nodes inside Klaviyo.
...
- [ ] Pass `?severity=high` â€” confirm only high-severity changes appear (zero in this case).
...
- [ ] Record the same change twice passing the same explicit `changeId`. Confirm the dashboard merges them into a single row with combined recipients (no duplicate recipient entries â€” last write wins per email).
...
## 8. Test Pass â€” Phase 5 (Post-Cruise + Alumni)
...
- [ ] Add a second past campaign that shares one guest with the first source. Re-fire live. Confirm `skippedDuplicateRecipient â‰¥ 1` and the shared guest received exactly ONE invite total (no duplicate inbox copies).
...
- [ ] For each test lead, confirm the expected ledger types in order: `waitlist_submitted` â†’ `nurture_queued` â†’ `nurture_sent` / `threshold_met_notified` / `booking_link_sent` / `expired`.
...
## 10. Inbound Email Parser (Booking Confirmation) *â€” relates to Phase 3*
...
- [ ] Inspect Klaviyo deliverability reporting for the test sends â€” bounce rate, complaint rate.
...
- [ ] Confirm UTM parameters on every email CTA (template-author concern â€” verify here).
...
- [ ] Cross-reference with the conversion dashboard â€” the test lead should be visible with the right `sourceChannel` and `provider` stamps.
...
- [ ] Every checkbox in Â§Â§4â€“9 is checked for at least one lead per relevant booking mode.
- [ ] No `lead_error` ledger entries are left unexplained.
- [ ] All Klaviyo flows under test are confirmed enabled with the pinned template versions captured in Â§2.1.
...
- [ ] What is the rate-limit ceiling we test against in Â§9.5?
- [ ] Who signs off â€” Engineering, Operations, or both?
