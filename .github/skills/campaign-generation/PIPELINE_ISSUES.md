...
### Research Dossier Timing Drift
- **Symptom:** The brief looks directionally fine, but the Production Bible or downstream scene prompts still feel generic or under-grounded in the niche even though a research dossier exists.
- **Root Cause:** The dossier was generated after the brief bundle, so the first Production Bible did not get to consume it. In the current engine, the dossier is only truly advantageous upstream if it exists before the brief bundle or if the brief/Production Bible is regenerated after the dossier is saved.
- **Mitigation:**
  - Prefer dossier-first sequencing when the campaign is at the start of brief work.
  - If the dossier is created later, regenerate the Production Bible or full brief bundle before approving for media.
  - Treat the dossier as a real creative input, not just a gating document.
...
- **Canva/Templated ad exception:** Static ads are no longer treated as full AI-rendered scenes. The forward path renders Canva/Templated layouts from Copy Forge copysets and manifest source images, with one preserved premium legacy display template kept as a first-class final ad.
- **No ad-in-ad rule:** Final designed ad artifacts must never be selected as source images for another static ad. If an ad appears inside a carousel, inspect the source-image pool and remove `manifest.images.designedAdArtifacts` from candidate still imagery.
- **Canva Ads tab semantics:** The review UI's Canva/Designed Ads surface should primarily show `manifest.images.designedAdArtifacts` as final ad outputs. `manifest.images.documentaryDetails` are optional audit/source modules, not part of the default all-media bundle.
