...
import { processingCoreLogic } from "./core-logic";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await processingCoreLogic({
      clients: ctx.clients,
    ...
    const detail = error instanceof Error ? error.message : "Processing start failed";
