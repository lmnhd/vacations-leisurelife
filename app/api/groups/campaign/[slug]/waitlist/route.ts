...
function buildNextStep(
  ...
  const retailBookingLink = odysseusRetailBookingLink ?? null;
  if (status === "EXPIRED") {
    return {
  ...
  if (
    (status === "THRESHOLD_MET" || status === "CONVERTED") &&
    ...
    return {
      ...
      detail:
        "The group threshold has been met, so you can move directly to the live booking path now.",
      bookingLink: retailBookingLink,
    };
  }
  ...
  return {
    ...
    title:
      bookingMode === "BOOK_NOW"
        ? "Your booking intent is saved."
        : "You are on the group wait path.",
    detail:
      bookingMode === "BOOK_NOW"
    ...
    bookingLink: bookingMode === "BOOK_NOW" ? retailBookingLink : bookingLink ?? null,
  };
}
...
export async function POST(
  ...
  if (campaign.status === "DRAFT" && !previewCaller) {
    return NextResponse.json(
      {
        success: false,
        error: "This campaign is not accepting public interest yet.",
      },
      { status: 409 },
    );
  }
    ...
    return NextResponse.json(
      {
        success: false,
        error:
          "SMS consent is required before we can save a phone number for threshold alerts.",
      },
      { status: 400 },
    );
  }
  ...
  // Derive a persistent guest token the browser can store in localStorage.
  // Encoding is base64url(slug:email) — readable server-side for display-name
  // attribution but opaque enough to prevent trivial guessing of other entries.
