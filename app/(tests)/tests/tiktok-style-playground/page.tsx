...
const SAMPLE_IMAGE_URL =
  "https://images.unsplash.com/photo-1552820728-8b83bb6b773f?w=400&h=600&fit=crop"; // Sample vertical image
...
export default function TikTokStylePlaygroundPage() {
  ...
  const loadCampaignBrief = async () => {
    ...
    try {
      const response = await fetch(
        `/api/groups/campaign/${slug}/media/aesthetic`,
        { cache: "no-store" },
      );
      if (!response.ok) {
      ...
      setBrief(
        (data as { brief?: CampaignAestheticBrief })?.brief ??
          (data as CampaignAestheticBrief),
      );

      const manifestResponse = await fetch(
        `/api/groups/campaign/${slug}/media/manifest`,
        { cache: "no-store" },
      );
  ...
  const updatePlacement = (
    placementUpdates: Partial<OverlayCardSpec["placement"]>,
  ) => {
    setPlayground((prev) => ({
      ...prev,
      customCard: {
  ...
  return (
    <div className="p-6 space-y-8 text-slate-100">
      <header className="space-y-4">
        <div>
          ...
          <p className="max-w-3xl text-sm text-slate-400">
            Experiment with TikTok video styles, overlay cards, and text
            positioning. Generate preview snippets without creating full videos.
          </p>
        </div>
        ...
        <div className="grid max-w-4xl gap-3 sm:grid-cols-[1fr_auto]">
          <label className="space-y-2 text-sm text-slate-300">
            ...
            <input
              ...
              className="w-full px-3 py-2 border rounded outline-none border-slate-700 bg-slate-950 text-slate-100 focus:border-cyan-500"
            />
          </label>
          <button
            type="button"
            ...
            disabled={loading}
            className="inline-flex items-center justify-center px-4 py-2 text-sm font-semibold transition rounded bg-cyan-500 text-slate-950 hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      {error && (
        <div className="p-4 text-sm text-red-200 border border-red-500 rounded bg-red-950/50">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_400px]">
        ...
        <div className="space-y-6">
          <div className="p-5 border rounded border-slate-700 bg-slate-950/80">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold transition rounded bg-emerald-500 text-slate-950 hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-50"
              >
            ...
            <div className="relative max-w-sm mx-auto overflow-hidden border rounded-lg border-slate-600 bg-slate-900">
              <img
              ...
              <div
                className="absolute p-6 border rounded-lg shadow-2xl border-yellow-400/50 bg-slate-900/80"
                style={{
              ...
              >
                <div
                  className="absolute top-0 left-0 w-full h-2 rounded-t-lg opacity-95"
                  style={{ backgroundColor: playground.customCard.accentColor }}
                />
                <div
                  className="mb-3 font-mono text-sm tracking-wider uppercase"
            ...
            {playground.generatedSnippet && (
              <div className="p-4 mt-4 border rounded border-emerald-500 bg-emerald-950/20">
                ...
                <video
                  ...
                  controls
                  className="w-full h-32 mt-2 border rounded border-slate-600"
                />
              </div>
            )}
          </div>
                ...
                >
                  ...
                  <div className="text-sm">
                    ...
                    <div className="text-slate-400">
                      {format.shotCount} shots • {format.targetDurationSeconds}s
                      • {format.distributionTag}
                    </div>
                  </div>
                </label>
              ))}
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(presetStyles).map(([key, preset]) => (
                  <button
                    key={key}
                    onClick={() =>
                      applyPreset(key as keyof typeof presetStyles)
                    }
                    className="px-3 py-2 text-xs text-left transition border rounded border-slate-600 bg-slate-800 hover:bg-slate-700"
                  >
                    <div className="font-medium capitalize text-slate-100">
                      {key}
                    </div>
            ...
            <div className="space-y-4">
              ...
              <label className="block space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateCustomCard({ headline: e.target.value })
                  }
                  className="w-full px-3 py-2 border rounded outline-none border-slate-700 bg-slate-950 text-slate-100 focus:border-cyan-500"
                  maxLength={46}
                />
              ...
              <label className="block space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateCustomCard({ subline: e.target.value })
                  }
                  className="w-full px-3 py-2 border rounded outline-none border-slate-700 bg-slate-950 text-slate-100 focus:border-cyan-500"
                />
              </label>
              ...
              <label className="block space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateCustomCard({ accentColor: e.target.value })
                  }
                  className="w-full h-10 border rounded border-slate-700 bg-slate-950"
                />
              </label>

              <div className="pt-2 space-y-3 border-t border-slate-700">
                <div className="flex items-center gap-3">
                  <input
                    ...
                    onChange={(e) =>
                      setPlayground((prev) => ({
                        ...prev,
                        applyFilmGrain: e.target.checked,
                      }))
                    }
                    className="text-cyan-500"
                  />
                ...
                {playground.applyFilmGrain && (
                  <label className="block space-y-2">
                    <span className="text-sm font-medium text-slate-300">
                      Grain Strength: {playground.grainStrength}
                    </span>
                    <input
                      type="range"
                      min="1"
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className="block space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updatePlacement({ x: Number(e.target.value) })
                  }
                  className="w-full px-3 py-2 border rounded outline-none border-slate-700 bg-slate-950 text-slate-100 focus:border-cyan-500"
                />
              </label>
              ...
              <label className="block space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updatePlacement({ y: Number(e.target.value) })
                  }
                  className="w-full px-3 py-2 border rounded outline-none border-slate-700 bg-slate-950 text-slate-100 focus:border-cyan-500"
                />
              </label>
              ...
              <label className="block space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updatePlacement({ width: Number(e.target.value) })
                  }
                  className="w-full px-3 py-2 border rounded outline-none border-slate-700 bg-slate-950 text-slate-100 focus:border-cyan-500"
                />
              </label>
              ...
              <label className="block space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updatePlacement({ height: Number(e.target.value) })
                  }
                  className="w-full px-3 py-2 border rounded outline-none border-slate-700 bg-slate-950 text-slate-100 focus:border-cyan-500"
                />
              </label>
