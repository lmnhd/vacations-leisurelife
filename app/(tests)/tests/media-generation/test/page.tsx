...
function getLatestAsset(
  ...
  return (
    selectPreferredAssetForContext(records, "general_moodboard", manifest) ??
...
function copyToResult(copy: CampaignMediaManifest["copy"]): GeneratorResult {
  return copy
    ? createSuccessResult(copy as unknown as Record<string, unknown>, "")
    : makeResult();
}
...
function cropsToResult(
  platformCrops: CampaignMediaManifest["images"]["platformCrops"],
): GeneratorResult {
  ...
  return createSuccessResult(
    {
      ...
      scenes: sceneImages.map((r) => ({
        sceneId: r.tags.find((t) => t !== "scene") ?? r.assetId,
        url: r.url,
      })),
    },
    sceneImages[0]?.url ?? "",
...
function manifestToPersistedState(
  manifest: CampaignMediaManifest,
): PersistedTestPageState {
  const latestHero =
    selectPreferredAssetForContext(
  ...
  return {
    ...
    merch0Result: assetRecordToResult(
      getLatestAsset(manifest.merch.designs, manifest),
    ),
    ...
    runwayCountdownResult: assetRecordToResult(
      getLatestAsset(manifest.videos.countdown, manifest),
    ),
    runwayBrollResult: assetRecordToResult(
      getLatestAsset(manifest.videos.broll, manifest),
    ),
  };
}
...
function normalizePersistedState(rawState: string): PersistedTestPageState {
  ...
  return {
    heroImageUrl:
      typeof parsedState.heroImageUrl === "string"
        ? parsedState.heroImageUrl
        : emptyState.heroImageUrl,
    themeMusicSource:
      parsedState.themeMusicSource === "replicate" ? "replicate" : "default",
    shipReferenceResult:
      (parsedState.shipReferenceResult as GeneratorResult | undefined) ??
      emptyState.shipReferenceResult,
    heroResult:
      (parsedState.heroResult as GeneratorResult | undefined) ??
      emptyState.heroResult,
    conceptResult:
      (parsedState.conceptResult as GeneratorResult | undefined) ??
      emptyState.conceptResult,
    cropResult:
      (parsedState.cropResult as GeneratorResult | undefined) ??
      emptyState.cropResult,
    copyResult:
      (parsedState.copyResult as GeneratorResult | undefined) ??
      emptyState.copyResult,
    narrationResult:
      (parsedState.narrationResult as GeneratorResult | undefined) ??
      emptyState.narrationResult,
    hypeResult:
      (parsedState.hypeResult as GeneratorResult | undefined) ??
      emptyState.hypeResult,
    replicateResult:
      (parsedState.replicateResult as GeneratorResult | undefined) ??
      emptyState.replicateResult,
    merch0Result:
      (parsedState.merch0Result as GeneratorResult | undefined) ??
      emptyState.merch0Result,
    sceneImagesResult:
      (parsedState.sceneImagesResult as GeneratorResult | undefined) ??
      emptyState.sceneImagesResult,
    tiktokVoiceoverResult:
      (parsedState.tiktokVoiceoverResult as GeneratorResult | undefined) ??
      (parsedState.heygenTiktokResult as GeneratorResult | undefined) ??
      emptyState.tiktokVoiceoverResult,
    storyboardVideoResult:
      (parsedState.storyboardVideoResult as GeneratorResult | undefined) ??
      (parsedState.heygenExplainerResult as GeneratorResult | undefined) ??
      emptyState.storyboardVideoResult,
    runwayCountdownResult:
      (parsedState.runwayCountdownResult as GeneratorResult | undefined) ??
      emptyState.runwayCountdownResult,
    runwayBrollResult:
      (parsedState.runwayBrollResult as GeneratorResult | undefined) ??
      emptyState.runwayBrollResult,
  };
}
...
function ResultPanel({
  ...
  previewType: "image" | "audio" | "video" | "json" | "image-and-json" | "none";
}) {
  ...
  const shipReferenceCandidates = Array.isArray(result.data?.candidates)
    ...
    : [];
  const designedAds = Array.isArray(result.data?.ads)
    ? (result.data.ads as Array<{ imageUrl: string; altText?: string }>)
    : [];
    ...
    <div className="mt-3 rounded-lg border border-white/10 bg-slate-950/60 overflow-hidden">
      ...
      {expanded && (
        <div className="border-t border-white/5 p-3 space-y-3">
          ...
          {shipReferenceCandidates.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {shipReferenceCandidates.map((candidate) => (
                <div
                  key={`${candidate.imageUrl}-${candidate.category}`}
                  className="rounded-lg border border-white/10 bg-slate-900/60 overflow-hidden"
                >
                  <a
                  ...
                  <div className="p-3 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1 text-[9px]">
                      <span className="px-1.5 py-0.5 rounded-full border border-cyan-500/20 text-cyan-300 bg-cyan-500/5">
                        {candidate.category}
                      </span>
                      <span className="px-1.5 py-0.5 rounded-full border border-emerald-500/20 text-emerald-300 bg-emerald-500/5">
                        score {candidate.selectionScore}
                      </span>
                      <span className="px-1.5 py-0.5 rounded-full border border-white/10 text-slate-400 bg-white/5">
                        {candidate.width}×{candidate.height}
                      </span>
                    </div>
                    <div className="text-[9px] text-slate-500 break-words">
                      {candidate.query}
                    </div>
                    <div className="flex flex-col gap-1 text-[10px]">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-slate-400 underline break-all"
                      >
                        Source page
                      </a>
                    </div>
          ...
          {previewType === "image" &&
            result.data &&
            "crops" in result.data &&
            Array.isArray(result.data.crops) && (
              <div className="grid grid-cols-4 gap-2">
                {(
                  result.data.crops as Array<{
                    format: string;
                    width: number;
                    height: number;
                    cdnUrl: string;
                  ...
                  <div key={crop.format} className="space-y-1">
                    <img
                      src={crop.cdnUrl}
                      alt={crop.format}
                      className="rounded w-full aspect-video object-cover border border-white/10"
                    />
          ...
          {previewType === "video" && cdnUrl && (
            <video
              controls
              src={cdnUrl}
              className="w-full rounded-lg border border-white/10 max-h-64"
            />
          )}
...
export default function MediaGenerationTestPage() {
  ...
  const [themeMusicSource, setThemeMusicSource] = useState<
    "replicate" | "default"
  >("default");
  ...
  useEffect(() => {
    fetch("/api/groups/media/env-check")
      ...
      .catch(() => {
        /* silently fail — badges show missing */
      });
  }, []);
  ...
  const [replicateResult, setReplicateResult] =
    useState<GeneratorResult>(makeResult());
  const [merch0Result, setMerch0Result] =
    useState<GeneratorResult>(makeResult());
  const [sceneImagesResult, setSceneImagesResult] =
    useState<GeneratorResult>(makeResult());
  const [tiktokVoiceoverResult, setTiktokVoiceoverResult] =
    useState<GeneratorResult>(makeResult());
  const [storyboardVideoResult, setStoryboardVideoResult] =
    useState<GeneratorResult>(makeResult());
  const [runwayCountdownResult, setRunwayCountdownResult] =
    useState<GeneratorResult>(makeResult());
  const [runwayBrollResult, setRunwayBrollResult] =
    useState<GeneratorResult>(makeResult());
    ...
    async function hydrateState(): Promise<void> {
      ...
      try {
        const response = await fetch(
          `/api/groups/campaign/${trimmedSlug}/media/manifest`,
        );
        if (!response.ok) {
          return;
        }
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      getTestPageStateKey(trimmedSlug),
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        <div className="border border-white/10 rounded-xl p-4 bg-slate-900/50">
          ...
          <p className="text-xs text-slate-500 mt-1">
            Each card hits one API call. Test individually to verify results
            before running the full pipeline. Requires an approved brief.
            Production Bible path generates scene images + storyboard-driven
            video.
          </p>
        </div>
        ...
        <div className="border border-white/10 rounded-xl p-3 bg-slate-900/50 flex flex-wrap gap-2 items-center">
          ...
          {Object.keys(KEY_META).map((k) => (
            <KeyBadge key={k} apiKey={k} keyStatus={keyStatus} />
          ))}
        </div>
        ...
        <div className="border border-white/10 rounded-xl p-4 bg-slate-900/50">
          ...
          <CampaignSelector value={slug} onChange={(s) => setSlug(s)} />
        </div>
        ...
        <div className="border border-white/10 rounded-xl p-4 bg-slate-900/50">
          ...
          <select
            ...
            onChange={(event) =>
              setThemeMusicSource(
                event.target.value === "replicate" ? "replicate" : "default",
              )
            }
        ...
        <GeneratorCard
          ...
          onRun={() =>
            runGenerator(
              `${base}/audio`,
              { generator: "elevenlabs_narration" },
              setNarrationResult,
            )
          }
        />
        ...
        <GeneratorCard
          ...
          onRun={() =>
            runGenerator(
              `${base}/images`,
              { generator: "ship_reference_search" },
              setShipReferenceResult,
            )
          }
        />
        ...
        <div className="border border-white/10 rounded-xl p-4 bg-slate-900/50">
          <label className="text-[10px] text-slate-500 uppercase tracking-widest block mb-2">
            Real Hero Image CDN URL — used by all video generators below
          </label>
          <input
            type="text"
            value={heroImageUrl}
        ...
        <GeneratorCard
          ...
          onRun={() =>
            runGenerator(
              `${base}/video`,
              { generator: "tiktok_voiceover", heroImageUrl },
              setTiktokVoiceoverResult,
            )
          }
        />
        ...
        <GeneratorCard
          ...
          onRun={() =>
            runGenerator(
              `${base}/video`,
              {
                generator: "storyboard_video",
                deliverableId: "hero_explainer",
              },
              setStoryboardVideoResult,
            )
          }
        />
        ...
        <GeneratorCard
          ...
          onRun={() =>
            runGenerator(
              `${base}/video`,
              { generator: "runway_countdown", heroImageUrl },
              setRunwayCountdownResult,
            )
          }
        />
        ...
        <GeneratorCard
          ...
          onRun={() =>
            runGenerator(
              `${base}/video`,
              { generator: "runway_broll", heroImageUrl },
              setRunwayBrollResult,
            )
          }
        />
        ...
        <GeneratorCard
          ...
          previewType="image"
          onRun={() =>
            runGenerator(`${base}/merch`, { itemIndex: 0 }, setMerch0Result)
          }
        />
      </div>
    </div>
...
> = {
  cyan: {
    ...
    text: "text-cyan-400",
    btn: "bg-cyan-500/20 hover:bg-cyan-500/30 border-cyan-500/30 text-cyan-300",
  },
  teal: {
    bg: "bg-teal-500/5",
    border: "border-teal-500/20",
    text: "text-teal-400",
    btn: "bg-teal-500/20 hover:bg-teal-500/30 border-teal-500/30 text-teal-300",
  },
  emerald: {
    bg: "bg-emerald-500/5",
    border: "border-emerald-500/20",
    text: "text-emerald-400",
    btn: "bg-emerald-500/20 hover:bg-emerald-500/30 border-emerald-500/30 text-emerald-300",
  },
  amber: {
    bg: "bg-amber-500/5",
    border: "border-amber-500/20",
    text: "text-amber-400",
    btn: "bg-amber-500/20 hover:bg-amber-500/30 border-amber-500/30 text-amber-300",
  },
  purple: {
    bg: "bg-purple-500/5",
    border: "border-purple-500/20",
    text: "text-purple-400",
    btn: "bg-purple-500/20 hover:bg-purple-500/30 border-purple-500/30 text-purple-300",
  },
  pink: {
    bg: "bg-pink-500/5",
    border: "border-pink-500/20",
    text: "text-pink-400",
    btn: "bg-pink-500/20 hover:bg-pink-500/30 border-pink-500/30 text-pink-300",
  },
  violet: {
    bg: "bg-violet-500/5",
    border: "border-violet-500/20",
    text: "text-violet-400",
    btn: "bg-violet-500/20 hover:bg-violet-500/30 border-violet-500/30 text-violet-300",
  },
  orange: {
    bg: "bg-orange-500/5",
    border: "border-orange-500/20",
    text: "text-orange-400",
    btn: "bg-orange-500/20 hover:bg-orange-500/30 border-orange-500/30 text-orange-300",
  },
  slate: {
    bg: "bg-slate-500/5",
    border: "border-slate-500/20",
    text: "text-slate-400",
    btn: "bg-slate-700/60 hover:bg-slate-700 border-slate-500/30 text-slate-400",
  },
};
...
function GeneratorCard({
  ...
  return (
    <div className={`border ${c.border} rounded-xl overflow-hidden ${c.bg}`}>
      {/* Header - always visible, left side clickable to toggle */}
      <div className="flex items-start justify-between gap-4 p-4">
        <div
          className="flex-1 space-y-1.5 min-w-0 cursor-pointer"
          onClick={() => setExpanded((v) => !v)}
        >
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {cost && (
              <span className="text-[9px] text-slate-500 border border-white/10 rounded px-1.5 py-0.5">
                {cost}
              </span>
            )}
            {!hasAllKeys && (
        ...
        <div className="flex items-center gap-2 shrink-0">
          {expanded && (
            <button
              ...
              onClick={onRun}
              disabled={isLoading}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium border ${c.btn} transition-all disabled:opacity-50 disabled:pointer-events-none`}
            >
              {isLoading ? (
      ...
      {expanded && (
        <>
          {apiKeys.length > 0 && (
            <div className="border-t border-white/5 px-4 py-2 flex flex-wrap gap-1">
              {apiKeys.map((k) => (
                <KeyBadge key={k} apiKey={k} keyStatus={keyStatus} />
              ))}
            </div>
