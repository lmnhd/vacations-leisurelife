...
export async function saveDirective(directive: CampaignDirective): Promise<void> {
...
export async function getDirective(
    ...
    const parsed = CampaignDirectiveSchema.parse(JSON.parse(result.Item.directiveJson as string));
    return {
        ...parsed,
        status: (result.Item.status as CampaignDirective['status']) ?? parsed.status,
        appliedAt: typeof result.Item.appliedAt === 'string' ? result.Item.appliedAt : parsed.appliedAt,
        failureReason: typeof result.Item.failureReason === 'string' ? result.Item.failureReason : parsed.failureReason,
    };
}

export async function listDirectives(slug: string): Promise<CampaignDirective[]> {
    ...
    return (result.Items ?? [])
        .map((item) => {
            try {
                const parsed = CampaignDirectiveSchema.parse(JSON.parse(item.directiveJson as string));
                return {
                    ...parsed,
                    status: (item.status as CampaignDirective['status']) ?? parsed.status,
                    appliedAt: typeof item.appliedAt === 'string' ? item.appliedAt : parsed.appliedAt,
                    failureReason: typeof item.failureReason === 'string' ? item.failureReason : parsed.failureReason,
                };
            } catch {
                return null;
            }
        })
        .filter((d): d is CampaignDirective => d !== null)
